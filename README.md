# DIGITAL-LITERACY-PROJECT

## Student Details
- Name: Ritambhar Advait
- Course: B.Tech (First Year)
- Branch: CSE (Cyber Security and Digital Forensics)
- Reg no.: 25BCY10086
- Course Code: CSE0001
- University: VIT Bhopal University


##  Project Overview
This Digital Literacy Project was completed as part of the CSE0001 course. The aim of this project is to develop essential digital skills, including creating a professional online presence, using online platforms effectively, communicating professionally, and understanding cyber safety.

The project consists of five tasks, each focusing on a different aspect of digital literacy.


##  Repository Structure

###  Task 1 – Digital Literacy Infographic
- Created an infographic using Canva
- Covered topics like digital literacy, useful degital tools for student, safe internet practices.
- Added screenshot of design in `task-1-presentation`
- canva link: https://canva.link/l1npqx1rw3j7ifh

---

###  Task 2 – Student Digital Portfolio
- Created profiles on:
  - GitHub ( special Profile README created)
  - LinkedIn
  - ResearchGate
- Added screenshots in `task-2-portfolio`
- Learned importance of professional digital identity

---

### Task 3 – Platforms Exploration
- Completed a basic  python coding challenge on HackerRank
- Created a Google Form for Digital Literacy Awareness Quiz
- Added screenshots of:
  - HackerRank challenge complition
  - hackerrank badge 
  - Google Form
  - google responce sheet with 5 responses.
- Files stored in `task-3-platforms`

---

###  Task 4 – Email Etiquette
- Wrote two professional emails:
  - Request for assignment extension
  - Internship inquiry email
- Created a Sociel Media Do’s and Don’ts checklist
- Files stored in `task-4-email-etiquette`

---

### Task 5 – Cybercrime Awareness
- Created a case study on UPI/Online Payment Fraud
- Developed a prevention checklist with safety tips
- Included reporting methods (cybercrime.gov.in, helpline 1930)
- Files stored in `task-5-cybercrime`

---

##  Google Form Link
Digital Literacy Awareness Quiz:  
https://docs.google.com/forms/d/e/1FAIpQLSd_WsBAOtIYtwBxm8EiPEGAgwy-TDgCeKjMxSx3aFR5r4ilww/viewform?usp=header

## Linkedin profile link
www.linkedin.com/in/ritambhar-advait-0b3b2137b

---

##  Key Learnings
- Importance of maintaining a professional online presence
- Basic understanding of coding platforms like HackerRank
- Writing professional emails with proper etiquette
- Awareness about cybercrime and online safety practices
- Using digital tools for collaboration and learning

---

##  Conclusion
This project helped me understand the importence of digital literacy in today’s world. I gained practicel experience in using various online platforms and tools, which will be helpful throughout my academic and profesional journey.

---
